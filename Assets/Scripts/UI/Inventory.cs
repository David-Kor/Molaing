...
public class Inventory : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        }    //처음에 인벤토리 슬롯들의 이미지가 전부 하얀 사각형이니 일괄적으로 비활성화.
    ...
    {
        //if (SaveNLoad.actLoad == true) return;
        for (int i = 0; i < slots.Length; i++)
        {
            if (slots[i].itemID.Equals(0))
            {
        ...
        for (int i = 0; i < eSlots.Length; i++)
        {
            ...
            else
            {
                ...
                eSlots[i].transform.GetChild(1).gameObject.SetActive(true);
            }
        }
    }
