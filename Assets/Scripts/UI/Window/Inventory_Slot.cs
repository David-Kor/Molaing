...
public class Inventory_Slot : MonoBehaviour
{
    ...
    public bool CheckSlot(int id)
    {
        for(int i = 48; i >=0; i--)
        {
            if (thisObject.transform.parent.GetChild(i).GetComponent<Inventory_Slot>().itemID == id)
            {
                thisObject.transform.parent.GetChild(i).GetComponent<Inventory_Slot>().Amount--;
                if(thisObject.transform.parent.GetChild(i).GetComponent<Inventory_Slot>().Amount <= 0)
                {
                    RemoveItem();
                }
                return true;
